...
pub fn get_month_beginning(year: u16, month: u16) -> i64 {
    ...
    let y = year + 4800 - a;
    let m = month + 12 * a - 3;
    ((153 * m + 2) / 5) as i64 + 365 * y as i64 + (y / 4) as i64 - (y / 100) as i64 + (y / 400) as i64 - 32044
}
...
pub fn generate_month_layout(year: i32, month: i32, output: &mut [[char; 21]; 8]) {
    ...
    // 复制月份头部
    for (i, c) in head[(month - 1) as usize].chars().enumerate() {
        output[0][i] = c;
    }
    ...
    let tail_length = 126 - 3 * (head_length + month_length);
